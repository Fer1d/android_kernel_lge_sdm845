...
do_gc:
		...
		if (f2fs_gc(sbi, sbi->rapid_gc || test_opt(sbi, FORCE_FG_GC),
...
/*
 * Upstream implements f2fs_lookup_rb_tree_ext()/f2fs_check_rb_tree_consistence()
 * generically over struct rb_entry, relying on the 64-bits 'key' union member
 * that this tree (based on 4.9) does not have.  The ATGC victim tree is the only
 * user, and it is keyed by victim_entry.mtime, so keep the helpers local here.
 */
static struct rb_node **atgc_lookup_victim_node(struct rb_root_cached *root,
...
static bool atgc_check_victim_tree_consistence(struct f2fs_sb_info *sbi)
{
#ifdef CONFIG_F2FS_CHECK_FS
	struct rb_node *cur = rb_first_cached(&sbi->am.root), *next;
				...
				unsigned long long mtime, unsigned int segno)
{
	...
	p = atgc_lookup_victim_node(&am->root, &parent, mtime, &left_most);
						...
						struct victim_sel_policy *p)
{
	...
	atgc_lookup_victim_node(&am->root, &parent, p->age, &left_most);
...
static void lookup_victim_by_age(struct f2fs_sb_info *sbi,
						struct victim_sel_policy *p)
{
	f2fs_bug_on(sbi, !atgc_check_victim_tree_consistence(sbi));
