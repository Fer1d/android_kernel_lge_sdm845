...
static unsigned int low_compress_ratio = 75;
...
/*
 * Parse "key=value" pairs, in-place. Kernel 4.14+ has next_arg() in
 * lib/cmdline.c; provide a local copy for this 4.9 tree.
 */
		...
		struct device_attribute *attr, const char *buf, size_t len)
{
	...
	if (kstrtouint(buf, 10, &value) || value > 100)
		return -EINVAL;

	low_compress_ratio = value;
	return len;
}
...
out:
	...
	if (flags) {
		...
		if (low_compress_ratio &&
		    (100 * (PAGE_SIZE - comp_len) / PAGE_SIZE) <
				low_compress_ratio) {
...
static void __zram_make_request(struct zram *zram, struct bio *bio)
{
	...
	start_time = jiffies;
	generic_start_io_acct(bio_op(bio), bio_sectors(bio),
			      &zram->disk->part0);
	...
	generic_end_io_acct(bio_op(bio), &zram->disk->part0, start_time);
	bio_endio(bio);
}
		       ...
		       struct page *page, bool is_write)
{
	...
	start_time = jiffies;
	generic_start_io_acct(is_write ? REQ_OP_WRITE : REQ_OP_READ,
			      SECTORS_PER_PAGE, &zram->disk->part0);
	ret = zram_bvec_rw(zram, &bv, index, offset, is_write, NULL);
	generic_end_io_acct(is_write ? REQ_OP_WRITE : REQ_OP_READ,
			    &zram->disk->part0, start_time);
out:
	/*
